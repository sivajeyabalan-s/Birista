...
const VEG_KEYS = new Set([
  'veg_appetizers', 'indian_bread', 'boba_tea',
  'lemonade', 'real_cream_bowl', 'desserts', 'milk_shakes',
]);

const TABS = [
  { id: 'all',        label: "Today's Exclusive", keys: null },
  { id: 'appetizers', label: 'Appetizers',         keys: ['non_veg_appetizers', 'veg_appetizers'] },
  { id: 'biryanis',   label: 'Biryanis',           keys: ['biryanis', 'family_pack_biryanis'] },
  { id: 'pulavs',     label: 'Pulavs',             keys: ['pulavs'] },
  { id: 'specials',   label: 'Specials',           keys: ['hyderabad_specials', 'ulvacharu_specials', 'gongura_specials'] },
  { id: 'breads',     label: 'Breads',             keys: ['indian_bread'] },
  { id: 'desserts',   label: 'Desserts',           keys: ['desserts', 'real_cream_bowl'] },
  { id: 'drinks',     label: 'Drinks',             keys: ['boba_tea', 'lemonade', 'milk_shakes'] },
];
...
const FEATURED_KEYS = [
  'biryanis', 'non_veg_appetizers', 'pulavs',
  'ulvacharu_specials', 'gongura_specials', 'desserts', 'boba_tea',
];

function getTabCount(tab, menu) {
  if (tab.id === 'all') {
    return FEATURED_KEYS.reduce(
      (sum, key) => sum + Math.min(2, menu[key]?.items?.length || 0), 0
    );
  }
  return (tab.keys || []).reduce(
    (sum, key) => sum + (menu[key]?.items?.length || 0), 0
  );
}
...
function MenuCard({ item, categoryKey }) {
  ...
  const isVeg    = VEG_KEYS.has(categoryKey);
    ...
    <article className="mp-card">
      <div className="mp-card-head">
        <div className="mp-tags">
          <span className={`mp-tag ${isVeg ? 'veg' : 'non-veg'}`}>
      ...
      <div className="mp-card-foot">
        ...
        <div className="mp-price">{currency} {item.price.toFixed(2)}</div>
      </div>
    </article>
  );
}
...
export default function MenuPage() {
  ...
  const sections = useMemo(() => {
    const keysToShow = activeTab === 'all'
      ? FEATURED_KEYS
    ...
    <div className="menu-page">
      ...
      <section className="mp-hero">
        ...
        <div className="mp-hero-content">
          <h1 className="mp-hero-title">Welcome to<br />Birista House</h1>
          <p className="mp-hero-sub">
            Experience the authentic taste of tradition with our rich, aromatic biryanis,
            sizzling kebabs, and handcrafted delicacies. Made with passion, served with warmth.
          </p>
        </div>
      ...
      <div className="mp-filter-row">
        <label className="mp-search-box">
          ...
          />
          {search && (
            <button className="mp-clear" onClick={() => setSearch('')}>×</button>
          )}
        </label>
      ...
      <div className="mp-cats-wrap">
        ...
        <button
          ...
          onClick={() => catsRef.current?.scrollBy({ left: 300, behavior: 'smooth' })}
        >
